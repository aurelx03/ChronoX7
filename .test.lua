...
local function DrawChrono()
			lcd.drawTimer(x+20, y+30, tableau[1], MIDSIZE)
 			...
 			--lcd.drawText(5,20,buffer)
end

local function Chrono()
local race = model.getTimer(0) --getChrono Race
local tour = model.getTimer(1) --getChrono Tour
	if tour.value < 1 and race.value > 5 and nbtour < 1 then
		tableau[1] = race.value
		nbtour = 1
	end
	if tour.value < 1 and race.value > 1 and nbtour == 1 and buffer > 150 then
		tableau[2] = race.value - tableau[1]
		nbtour = 2
		buffer = 0
	end
	if tour.value < 1 and race.value > 1 and nbtour == 2 and buffer > 150 then
		tableau[3] = race.value - (tableau[1]+tableau[2])
		nbtour = 3
		buffer = 0
	end
	if tour.value < 1 and race.value > 1 and nbtour == 3 and buffer > 150 then
		tableau[4] = race.value - (tableau[1]+tableau[2]+tableau[3])
		nbtour = 4
		buffer = 0
	end
buffer = buffer + 1
...
local function BestLap()
	...
	tour = model.getTimer(1)
	race = model.getTimer(0)
	if tour.value < 1 and race.value < 2 then
			tableau[1] = 0
			tableau[2] = 0
			tableau[3] = 0
			tableau[4] = 0
